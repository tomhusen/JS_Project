...
function unitPoint(p) {
  ...
  var lenSqrd = p[0]*p[0] + p[1]*p[1] + p[2]*p[2];
  var result  = p;
 ...
 */
function divideTriangle( divs, a, b, c, points ) {
  var result;
  if (divs > 0) { // subdivide the triangle into four
    ...
    var divsMinusOne = divs - 1;
    divideTriangle(divsMinusOne, a, v1, v2, points);
    divideTriangle(divsMinusOne, c, v2, v3, points);
    divideTriangle(divsMinusOne, b, v3, v1, points);
    result = 4 * divideTriangle(divsMinusOne, v1, v3, v2, points);
